...
DROP POLICY IF EXISTS "customers access own data" ON customers;
CREATE POLICY "customers access own data" ON customers
    FOR SELECT USING (auth.uid() = customer_id OR role = 'admin');

DROP POLICY IF EXISTS "matches access own data" ON matches;
CREATE POLICY "matches access own data" ON matches
    FOR ALL USING (customer_id IN (
        SELECT c.customer_id FROM customers c
        WHERE c.customer_id = auth.uid() OR c.role = 'admin'
    ));

DROP POLICY IF EXISTS "events access own matches" ON events;
...
DROP POLICY IF EXISTS "transactions access own data" ON transactions;
CREATE POLICY "transactions access own data" ON transactions
    FOR ALL USING (customer_id = auth.uid() OR customer_id IN (
